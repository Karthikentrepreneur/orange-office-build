          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.label}
                to={item.to}
                className={`font-medium transition-colors duration-300 relative text-gray-800 hover:text-orange-500
                  after:content-[''] after:absolute after:w-full after:scale-x-0 after:h-0.5 after:bottom-0 after:left-0 
                  after:origin-bottom-right after:transition-transform after:duration-300 hover:after:scale-x-100 
                  hover:after:origin-bottom-left after:bg-orange-500 ${
                    location.pathname === item.to ? "text-orange-500 after:scale-x-100 after:bg-orange-500" : ""
                  }`}
              >
                {item.label}
              </Link>
          ...
          <nav className="container mx-auto px-4 py-4 flex flex-col space-y-3">
            {navItems.map((item) => (
              <Link
                key={item.label}
                to={item.to}
                className={`text-base text-gray-800 hover:text-orange-500 font-medium py-2 transition-colors duration-300 ${
