...
export const Hero = ({
  ...
  return (
    <section className="relative overflow-hidden min-h-[100vh] flex flex-col justify-between bg-white">
      {/* Background Carousel */}
      ...
      {/* Overlay Content */}
      <div className="absolute inset-0 bg-black/30 z-10 flex flex-col justify-center items-center px-4">
        {/* Heading */}
        <h1 className="text-3xl md:text-5xl font-bold text-white text-center drop-shadow mb-4">
          Empowering Logistics with Smart Solutions
        </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          {services.map((service, idx) => (
            <Button
              key={idx}
              variant="secondary"
              className="bg-white/90 text-orange-600 font-semibold hover:bg-orange-500 hover:text-white transition px-4 py-2 rounded-lg shadow"
            >
              {service}
            </Button>
          ))}
